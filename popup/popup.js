...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    subtitleBtn.addEventListener('click', async () => {
        ...
        try {
            // 检查 Groq Key
            const groqCheck = await chrome.runtime.sendMessage({ type: 'CHECK_GROQ_KEY' });
            if (!groqCheck.hasKey) {
                showStatus(subtitleStatus, '需要 Groq API Key，请在设置中配置', 'error');
            ...
            if (response.status === 'started') {
                ...
                showStatus(subtitleStatus, '音频捕获中...', 'active');
            } else if (response.status === 'stopped') {
    ...
    async function checkApiKey() {
        try {
            ...
            if (!response.hasKey) {
                ...
                apiStatusText.textContent = '未配置 Gemini API Key';
                apiStatusText.className = 'api-status-text error';
                return false;
            }

            // Gemini Key 存在，但检查 Groq Key 并提示（不阻止）
            const groqRes = await chrome.runtime.sendMessage({ type: 'CHECK_GROQ_KEY' });
            if (!groqRes.hasKey) {
                showStatus(subtitleStatus, '未配置 Groq Key，语音字幕不可用', '');
            }
